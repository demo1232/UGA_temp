...
import com.ncsavault.alabamavault.dto.AssigneeDto;
...
import com.ncsavault.alabamavault.dto.TaskDto;
import com.ncsavault.alabamavault.dto.User;
import com.ncsavault.alabamavault.dto.VideoDTO;
import com.ncsavault.alabamavault.globalconstants.GlobalConstants;
import com.ncsavault.alabamavault.service.BusinessException;
...
//
//            url = url.replaceAll(" ", "%20");
//            ArrayList<VideoDTO> videoList1 = new ArrayList<>();
//            try {
//                String result = getData(url);
////        System.out.println("Response of url : "+result);
//                if (result == null) {
...
//        }
//
//        System.out.println("Size of video list : " + videoList1.size());
//        VideoDTO vidObj;
//        for (Iterator<VideoDTO> it = videoList1.iterator(); it.hasNext(); ) {
//            vidObj = it.next();
//            if (vidObj.getVideoName().equals("") && vidObj.getVideoShortDescription() == "")
//                it.remove();
//        }
//        System.out.println("Updated Size of video list : " + videoList1.size());
        return videoList;
    }
...
//                    return null;
    ...
    public ArrayList<CatagoriesTabDao> getCategoriesData(String url) throws BusinessException {
        try {
            //
            url = url.replaceAll(" ", "%20");
            ArrayList<CatagoriesTabDao> catagoriesList = new ArrayList<>();
            try {
                String result = getData(url);
//        System.out.println("Response of url : "+result);
                if (result == null) {
//            if (result.contains("timeout"))
            ...
            Type classType = new TypeToken<ArrayList<CatagoriesTabDao>>() {
            }.getType();
            ArrayList<CatagoriesTabDao> response = gson.fromJson(result.trim(), classType);
            catagoriesList = response;
        } catch (Exception e) {
            e.printStackTrace();
        }

        System.out.println("Size of video list : " + catagoriesList.size());
...
//            if (result.contains("timeout"))
                ...
                ArrayList<PlaylistDto> response = gson.fromJson(result.trim(), classType);
                playlistDtoArrayList = response;
            } catch (Exception e) {
                e.printStackTrace();
            }
...
//            if (result.contains("timeout"))
                ...
                ArrayList<VideoDTO> response = gson.fromJson(result.trim(), classType);
                videoDTOArrayList = response;
            } catch (Exception e) {
                e.printStackTrace();
            }

            System.out.println("Size of video list : " + videoDTOArrayList.size());
            System.out.println("Updated Size of video list : " + videoDTOArrayList.size());
        ...
        try {
            ...
            if (inputStream != null) {
                try {
                    ...
                    System.out.println("result favorite : "+result);
                    return result;
                } catch (IOException e) {
                    e.printStackTrace();
                }
...
//                    return null;
    ...
    public String getData(String serverUrl) throws Exception {
        URL url;
        ...
        try {
            ...
            connection.setRequestProperty("appID", String.valueOf(GlobalConstants.APP_ID));
            connection.setRequestProperty("appVersion", GlobalConstants.APP_VERSION);
            connection.setRequestProperty("deviceType", GlobalConstants.DEVICE_TYPE);
