...
//                        arrList.addAll(AppController.getInstance().getServiceManager().getVaultService().getVideosListFromServer(url));
                        ...
                        //gk favoriteVideoList.addAll(VaultDatabaseHelper.getInstance(mActivity.getApplicationContext()).getFavouriteVideosArrayList());
                ...
                protected void onPostExecute(ArrayList<VideoDTO> list) {
                    ...
                    //gk Utils.setVisibilityOfScrollBarHeightForNormalList(mActivity, GlobalConstants.SEARCH_VIEW_QUERY, listViewFavouriteVideos);
...
//            Utils.addBannerImage(bannerCacheableImageView, GlobalConstants.FAVORITES, mActivity);
                    ...
                    countDownTimer = new CountDownTimer(1000, 3000) {
                        ...
                        public void onFinish() {
                            ...
                            LocalModel localModel = AppController.getInstance().getModelFacade().getLocalModel();
...
//                    videoListAdapter.listSearch.addAll(favoriteVideoList);
                ...
                //gk Utils.setVisibilityOfScrollBarHeightForNormalList(mActivity, newText, listViewFavouriteVideos);
    ...
    public class PullRefreshTask extends AsyncTask<Void, Void, ArrayList<VideoDTO>> {
        ...
        protected ArrayList<VideoDTO> doInBackground(Void... params) {
            ArrayList<VideoDTO> arrList = new ArrayList<VideoDTO>();
            try {
                String url = GlobalConstants.FAVORITE_API_URL + "userId=" + AppController.getInstance().getModelFacade().getLocalModel().getUserId();
                arrList.addAll(AppController.getInstance().getServiceManager().getVaultService().getVideosListFromServer(url));
            } catch (Exception e) {
                e.printStackTrace();
            }
